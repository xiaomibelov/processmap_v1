...
TS="$(date +%F_%H%M%S)"
TAG="cp/fpc_diag_docker_code_mount_v4_${TS}"
git tag -a "$TAG" -m "checkpoint: diag docker code mount v4 (${TS})" >/dev/null 2>&1 || true
...
echo "inspect_json_bytes=$(wc -c <"$INSPECT_JSON" 2>/dev/null || echo 0)"
echo "inspect_err_bytes=$(wc -c <"$INSPECT_ERR" 2>/dev/null || echo 0)"
...
echo
echo "== docker inspect stdout head (first 30 lines) =="
sed -n '1,30p' "$INSPECT_JSON" 2>/dev/null || true
...
if not arr:
    print("EMPTY: inspect json array")
    raise SystemExit(0)
doc=arr[0]
...
PY
  ...
  echo "SKIP: inspect json is empty (see stderr/head above)"
fi
