...
TS="$(date +%F_%H%M%S)"
TAG="cp/fix_schema_init_subscriptions_ends_at_v2_start_${TS}"
git tag -a "$TAG" -m "checkpoint: fix schema init subscriptions ends_at v2 start (${TS})" >/dev/null 2>&1 || true
echo "checkpoint tag: $TAG"

cat > scripts/patch_schema_subscriptions_ends_at_v2.py <<'PY'
...
echo
echo "== run patcher =="
python scripts/patch_schema_subscriptions_ends_at_v2.py || true
