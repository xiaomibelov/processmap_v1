...
def patch_app_js() -> bool:
    ...
    marker = "STEP18C3F_ZOOM_CONTROLS"
    if marker in s:
        print("app.js: marker already present")
        return False
    ...
    addon = "\n".join([
        ...
        "      }",
        "      // after fit, base scale will be re-captured by wrapper and zoom applied",
        "    }",
        "",
        "    function buildCtl(){",
        "      var wrap = getWrap();",
        ...
        "      var svg = getSvg();",
        "      if (!svg) return;",
        "      // absolute percent is applied to base scale (fit result).",
        "      var base = window.__fpcMermaidBaseScale;",
        "      if (!(base > 0)) { base = parseScale(svg.style.transform) || 1.0; window.__fpcMermaidBaseScale = base; }",
        ...
        "        try {",
        "          window.__fpcFitMermaidOrig(cfg);",
        "        } catch(e) {}",
        "        try {",
        "          var svg = getSvg();",
        "          if (svg) {",
        ...
        "      ensureCfg();",
        "      wrapFitMermaid();",
        "      // Rebuild controller when Mermaid re-renders (innerHTML replaced)",
        "      var wrap = getWrap();",
        "      if (!wrap) return;",
        ...
        "      // initial",
        "      buildCtl();",
        "      // if fit already ran before controller init",
    ...
    print("app.js: appended zoom controller")
    return True

def patch_styles_css() -> bool:
    p = Path("backend/app/static/styles.css")
    s = p.read_text(encoding="utf-8", errors="replace")
    marker = "STEP18C3F_ZOOM_CONTROLS_CSS"
    if marker in s:
        print("styles.css: marker already present")
        return False
    ...
    print("styles.css: appended zoom controls css")
    return True
