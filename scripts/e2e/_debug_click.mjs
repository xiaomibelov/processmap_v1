...
async function run() {
  ...
  // add our own capture listener to log
  await page.evaluate(() => {
    const m = window.__FPC_E2E_MODELER__;
    if (!m) return;
    const container = m._container;
    container.addEventListener('click', (e) => {
      const btn = e.target?.closest?.('.bjs-drilldown');
      console.log('CAPTURE click target', e.target?.tagName, e.target?.className, 'button?', !!btn);
    }, true);
  });

  // click arrow
  const btn = await page.locator('.bjs-drilldown').first();
  await btn.click();
