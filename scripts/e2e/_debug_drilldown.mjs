...
async function run() {
  ...
  const info = await page.evaluate(() => {
    ...
    if (!m) return { noModeler: true };
    const container = m._container;
    ...
    return {
      ...
      containerHasBtn: !!container?.contains(btn),
      btnClass: btn?.className,
      btnParentClass: btn?.parentElement?.className,
      overlayCount: candidates.length,
      overlayElementIds: candidates.map(o => ({ id: o.element?.id, type: o.element?.type, htmlClass: o.html?.className })),
      listeners: container?._events ? Object.keys(container._events) : null,
    };
  });
