...
const NAV_STATUS_DOT_MAX = 760;
const NAV_BACK_ICON_MAX = 600;
  ...
  return await page.evaluate(
    ({ backTestId, crumbsTestId }) => {
      ...
      return {
        ...
        // элементы разной высоты центрируются в строке — сравниваем центры
        sameLine: Math.abs((b.y + b.height / 2) - (c.y + c.height / 2)) <= 1.5,
...
async function main() {
  ...
  for (const width of WIDTHS) {
    ...
    // ── Уровень 3: сессия (первая сессия проекта; ссылка может быть
    // вне зоны видимости — идём напрямую по href) ──
    await page.waitForSelector('a[href*="session="]', { state: "attached", timeout: 15000 });
    const sessionHref = await page.locator('a[href*="session="]').first().getAttribute("href");
    if (!sessionHref) throw new Error("session href not found");
    await page.goto(`${BASE_URL}${sessionHref}`);
