...
echo "[smoke:prod] checking running services"
docker compose "${COMPOSE_ARGS[@]}" ps api gateway postgres redis >/dev/null

echo "[smoke:prod] checking web root through internal gateway service"
...
print("[smoke:prod] web root OK")
PY

echo "[smoke:prod] checking api health and public invite boundary"
docker compose "${COMPOSE_ARGS[@]}" exec -T api python3 - <<'PY'
import json
import os
import urllib.error
...
print("[smoke:prod] api health OK")
print(f"[smoke:prod] invite resolve boundary OK (status={status})")
...
if os.environ.get("DEV_SEED_ADMIN") == "1" and os.environ.get("ADMIN_EMAIL") and os.environ.get("ADMIN_PASSWORD"):
    ...
    print("[smoke:prod] seeded admin auth/orgs OK")
PY

echo "[smoke:prod] completed"
