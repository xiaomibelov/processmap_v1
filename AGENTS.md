...
### 6.2. Правило секретов (absolute, контур fix/stage-secret-cleanup-v1)
- Пароли, токены, API-ключи, куки сессий НИКОГДА не коммитятся в git и не пишутся в отслеживаемые файлы: код, тесты, фикстуры, конфиги, docs, `.planning`-артефакты, миррор, Obsidian.
- Креды передаются агенту ТОЛЬКО в сессию владельцем. Агент использует их в памяти/локальных неотслеживаемых файлах и НЕ сохраняет в репо и артефакты.
- Код и тесты читают креды из env / внешних неотслеживаемых файлов (`FOO_PASSWORD`, `FOO_TOKEN`). Значения по умолчанию с реальными кредами запрещены; пусто → понятная ошибка, какие env нужны.
- НАЙДЕН СЕКРЕТ в tracked-файле или артефактах → немедленно, без ожидания approve: значение НЕ цитировать никуда; минимальный патч на env-чтение; вычистить копии в артефактах/worktree; зафиксировать находку без значения и запросить РОТАЦИЮ.
- Секрет, побывавший в git, считается скомпрометированным ВСЕГДА (история, клоны, бэкапы). Ротация обязательна; чистка файла недостаточна. Git-историю не переписывать без отдельного решения владельца.
- Гейты и автотесты, требующие логина, используют выделенные СЕРВИСНЫЕ учётки с минимальными правами, НЕ личные аккаунты людей.
- Перед каждым push/PR агент сам грепает свою дельту на паттерны секретов (`password`, `token`, `api_key`, `Bearer`, `Authorization` и т.п.); срабатывание → стоп, чистка, потом push.
- Нарушение в чужом/влитом коде фиксируется как находка и оформляется отдельным контуром чистки (`fix/secret-cleanup-*`), эскалация владельцу.
- Enforcement (CI secret-scan, pre-commit hook) — tooling-бэклог, см. handoff 2026-09-24.
