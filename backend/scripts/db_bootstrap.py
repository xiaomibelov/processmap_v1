...
# линейный порядок цепочки (001→002→004→003→005→006→007→008→009→010→011→...→033→034)
...
LINEAR = ["001", "002", "004", "003", "005", "006", "007", "008", "009", "010", "011", "012", "013", "014", "015", "016", "017", "018", "019", "020", "021", "022", "023", "024", "025", "026", "027", "028", "029", "030", "031", "032", "033", "034", "035", "036"]

# маркер «объект ревизии существует» (SELECT 1 ... LIMIT 1)
MARKERS = {
    # fix/agent-empty-schema-raw-json-v1: маркер 036 — active prompt v3 processman_agent (пустая схема).
    "036": "SELECT 1 FROM llm_prompts WHERE id='llmprompt_processman_agent_v3' AND status='active' LIMIT 1",
    ...
    "033": "SELECT 1 FROM llm_prompts WHERE id='llmprompt_agent_router_v3' AND status='active' LIMIT 1",
    ...
    "030": "SELECT 1 FROM llm_prompts WHERE feature='product_actions_suggest' AND version=2 AND status='active' LIMIT 1",
    # P0-stage-unblock: маркер 029 — active prompt v1 для product_actions_suggest (LLM gateway).
    "029": "SELECT 1 FROM llm_prompts WHERE feature='product_actions_suggest' AND version=1 AND status='active' LIMIT 1",
    # AGENT-OBS: маркер 028 — колонка summary в agent_conversations.
    "028": "SELECT 1 FROM information_schema.columns WHERE table_name='agent_conversations' AND column_name='summary' LIMIT 1",
    ...
    "017": "SELECT 1 FROM information_schema.tables WHERE table_name='agent_conversations' LIMIT 1",
    # LLM-model-config: маркер 016 — реестр моделей (структурный маркер).
    "016": "SELECT 1 FROM information_schema.tables WHERE table_name='llm_models' LIMIT 1",
