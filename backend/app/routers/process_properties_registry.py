...
from ..services import registry_core
...
# _text/_texts остаются в адаптере: _text импортируется analytics.py,
# обе используются локальными хелперами метаданных.
...
def _registry_payload(inp: ProcessPropertiesRegistryQueryIn, request: Request, *, paginate: bool = True) -> Dict[str, Any]:
    return registry_core.build_registry_payload(_REGISTRY_CORE_CONFIG, inp, request, paginate=paginate)
...
_REGISTRY_CORE_CONFIG = registry_core.RegistryCoreConfig(
    export_columns=list(_EXPORT_COLUMNS),
    filter_map=dict(_FILTER_MAP),
    filter_option_map=dict(_FILTER_MAP),
    filename_prefix="process-properties",
    xlsx_sheet_name="Process properties",
    xlsx_column_widths=[18, 22, 18, 24, 18, 18, 18, 16, 22, 22, 22, 18, 18, 18, 16, 12],
    source_label="process_properties_registry_backend",
    source_namespace="/api/analysis/properties/registry",
    source_contract_version="v1",
    sort_key=_sort_key,
    completeness=_completeness,
    load_sources=lambda **kwargs: get_storage().list_process_properties_registry_sources(**kwargs),
    extract_rows=_extract_camunda_rows,
)

query_process_properties_registry = registry_core.make_query_endpoint(
    _REGISTRY_CORE_CONFIG, "query_process_properties_registry", ProcessPropertiesRegistryQueryIn
)
export_process_properties_registry_csv = registry_core.make_export_csv_endpoint(
    _REGISTRY_CORE_CONFIG, "export_process_properties_registry_csv", ProcessPropertiesRegistryQueryIn
)
export_process_properties_registry_xlsx = registry_core.make_export_xlsx_endpoint(
    _REGISTRY_CORE_CONFIG, "export_process_properties_registry_xlsx", ProcessPropertiesRegistryQueryIn
)

router.post("/api/analysis/properties/registry/query")(query_process_properties_registry)
router.post("/api/analysis/properties/registry/export.csv")(export_process_properties_registry_csv)
router.post("/api/analysis/properties/registry/export.xlsx")(export_process_properties_registry_xlsx)
