"""Shared helpers for registry routers.

Common implementation for ``process_properties_registry`` and
``product_actions_registry``. Entity-specific behavior (row extraction,
filter maps, export layout, source-state labels) is injected via
``RegistrySpec`` / function parameters so both routers keep their exact
HTTP contracts.
"""
...
__all__ = [
    ...
    "_validate_project_ids",
    "_validate_session_ids",
...
class RegistrySpec:
    """Entity-specific configuration for a registry router."""

    filter_map: Dict[str, str]
    sort_keys: Sequence[str]
    list_sources: Callable[..., List[Dict[str, Any]]]
    extract_rows: Callable[[Dict[str, Any]], List[Dict[str, Any]]]
    export_file_prefix: str
    xlsx_column_widths: Sequence[int]
    xlsx_sheet_name: str
    source_state_source: str
    source_state_namespace: str
...
def _matches_filters(row: Dict[str, Any], filters: Any, filter_map: Dict[str, str]) -> bool:
...
def _sort_key(row: Dict[str, Any], keys: Sequence[str]) -> tuple:
    return tuple(_text(row.get(key)).lower() for key in keys)
...
def _filter_options(rows: List[Dict[str, Any]], filter_map: Dict[str, str]) -> Dict[str, List[str]]:
    options: Dict[str, Set[str]] = {filter_key: set() for filter_key in filter_map}
    options["completeness"] = {"all", "complete", "incomplete"}
    for row in rows:
        for filter_key, row_key in filter_map.items():
            value = _text(row.get(row_key))
            if value:
                options[filter_key].add(value)
    return {k: sorted(v) for k, v in options.items()}


def _applied_filters(filters: Any, filter_map: Dict[str, str]) -> Dict[str, Any]:
    completeness = _text(filters.completeness or "all").lower() or "all"
    if completeness not in {"all", "complete", "incomplete"}:
        raise HTTPException(status_code=422, detail="invalid completeness filter")
    applied: Dict[str, Any] = {filter_key: _texts(getattr(filters, filter_key, [])) for filter_key in filter_map}
    applied["completeness"] = completeness
    return applied
...
def _source_state(
    ...
    *,
    ...
    if source_contract_version is not None:
        state["source_contract_version"] = source_contract_version
    return state


def _session_summary(source: Dict[str, Any], extract_rows: Callable[[Dict[str, Any]], List[Dict[str, Any]]]) -> Dict[str, Any]:
    actions = extract_rows(source)
...
def _registry_payload(inp: Any, request: Request, spec: RegistrySpec, *, paginate: bool = True) -> Dict[str, Any]:
    ...
    session_summaries = [_session_summary(source, spec.extract_rows) for source in sources]
    all_rows: List[Dict[str, Any]] = []
    for source in sources:
        all_rows.extend(spec.extract_rows(source))
    session_summaries = _reconcile_session_summaries_with_rows(session_summaries, all_rows)

    applied_filters = _applied_filters(inp.filters, spec.filter_map)
    filter_options = _filter_options(all_rows, spec.filter_map)

    filtered_rows = [row for row in all_rows if _matches_filters(row, inp.filters, spec.filter_map)]
    filtered_rows.sort(key=lambda row: _sort_key(row, spec.sort_keys))
    ...
    empty_state = _empty_state(scope, all_rows, filtered_rows, session_summaries, applied_filters, tuple(spec.filter_map))
    source_state = _source_state(
        session_summaries,
        all_rows,
        sources,
        source=spec.source_state_source,
        namespace=spec.source_state_namespace,
...
def _export_filename(file_prefix: str, scope: str, extension: str) -> str:
    safe_scope = _normalize_scope(scope)
    stamp = datetime.now().strftime("%Y%m%d-%H%M")
    return f"{file_prefix}-{safe_scope}-{stamp}.{extension}"
...
def _xlsx_bytes(rows: List[Dict[str, Any]], columns: Sequence[str], column_widths: Sequence[int], sheet_name: str) -> bytes:
    output = io.BytesIO()
    sheet_rows = []
