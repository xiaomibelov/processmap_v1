...
# _text/_texts остаются в адаптере: используются локальными хелперами view-model.
...
_REGISTRY_CORE_CONFIG = registry_core.RegistryCoreConfig(
    ...
    filename_prefix="product-actions",
    xlsx_sheet_name="Product actions",
    xlsx_column_widths=[18, 22, 18, 24, 18, 18, 22, 18, 18, 22, 22, 18, 18, 24, 18, 18, 16, 16, 14, 12, 14, 22],
    source_label="product_actions_registry_backend",
    source_namespace="/api/analysis/product-actions/registry",
    source_contract_version=None,
    sort_key=_sort_key,
    completeness=lambda row: _completeness(row)[0],
    load_sources=lambda **kwargs: get_storage().list_product_action_registry_sources(**kwargs),
    extract_rows=_extract_rows,
)

query_product_actions_registry = registry_core.make_query_endpoint(
    _REGISTRY_CORE_CONFIG, "query_product_actions_registry", ProductActionsRegistryQueryIn
)
export_product_actions_registry_csv = registry_core.make_export_csv_endpoint(
    _REGISTRY_CORE_CONFIG, "export_product_actions_registry_csv", ProductActionsRegistryQueryIn
)
export_product_actions_registry_xlsx = registry_core.make_export_xlsx_endpoint(
    _REGISTRY_CORE_CONFIG, "export_product_actions_registry_xlsx", ProductActionsRegistryQueryIn
)

router.post("/api/analysis/product-actions/registry/query")(query_product_actions_registry)
router.post("/api/analysis/product-actions/registry/export.csv")(export_product_actions_registry_csv)
router.post("/api/analysis/product-actions/registry/export.xlsx")(export_product_actions_registry_xlsx)
