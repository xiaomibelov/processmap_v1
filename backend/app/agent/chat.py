"""Core turn execution for AGENT-0 PROCESSMAN chat."""
...
def _run_action(
    ...
    if action == "suggest-next":
        ...
        if after_step_id and after_step_id not in step_ids_set:
            return None
        return run_suggest_next(session_id, request, after_step_id=after_step_id)

    if action == "explain-step":
        step_id = str(payload_obj.get("step_id") or "").strip()
        if not step_id or step_id not in step_ids_set:
            return None
        return run_explain_step(session_id, request, step_id=step_id)
    ...
    if action == "step-qa":
        ...
        if not step_id or step_id not in step_ids_set:
            return None
        return run_step_qa(session_id, request, step_id=step_id, question=question)
...
def run_turn(
    ...
    ctx = load_context(sid, uid, oid, history_limit=50)
    ...
    if action_obj and isinstance(action_obj, dict):
        ...
        if possible_action in {"suggest-next", "explain-step", "step-qa"}:
            action_result = _run_action(possible_action, action_obj, sid, request, ctx, payload.message)
