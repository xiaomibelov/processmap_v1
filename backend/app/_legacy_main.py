...
def create_template_endpoint(inp: TemplateCreateIn, request: Request) -> Dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        return _enterprise_error(422, "validation_error", "payload must be object")
    ...
    if normalized_scope == "org":
        ...
        if err is not None:
            return err
        org_role = str(role or "").strip().lower()
        if not (is_admin or _is_role_allowed(org_role, _ORG_TEMPLATE_WRITE_ROLES)):
...
def patch_template_endpoint(template_id: str, inp: TemplatePatchIn, request: Request) -> Dict[str, Any]:
    ...
    if not (has_name or has_description or has_payload):
        return _enterprise_error(422, "validation_error", "empty_patch")
    if has_payload and not isinstance(inp.payload, dict):
        return _enterprise_error(422, "validation_error", "payload must be object")
...
def _template_can_manage(
    *,
    ...
    if scope == "org":
        return _is_role_allowed(org_role, _ORG_TEMPLATE_WRITE_ROLES)
    return False
