...
def test_empty_schema_smalltalk_raw_action_json_gets_fallback(admin_token, session_with_steps, mock_projection, mock_route_intent_smalltalk):
    """N1/M3: пустая схема + модель вернула сырой action-JSON -> дружелюбный текст, не JSON."""
...
def test_smalltalk_valid_action_json_executes_runner(admin_token, session_with_steps, mock_projection, mock_route_intent_smalltalk):
    """Валидный action-JSON в fence на непустой схеме выполняется как action, не free-answer."""
    mock_route_intent_smalltalk.return_value = "smalltalk"
    with mock.patch("memory.chat.complete") as fake_complete, \
         mock.patch("memory.chat.run_explain_step") as fake_runner:
        fake_complete.return_value = _ok_llm_result(
        ...
        fake_runner.return_value = {"ok": True, "status": "ok", "message": "Step 1: тестовое объяснение"}
        c = TestClient(app)
        r = c.post(
            f"/sessions/{session_with_steps}/agent/chat",
            headers=_auth(admin_token),
            json={"message": "объясни шаг"},
        )
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["ok"] is True
    assert body["action"] == "explain-step"
    assert body["message"] == "Step 1: тестовое объяснение"
    fake_runner.assert_called_once()
    assert fake_runner.call_args.kwargs.get("step_id") == "step_1"


def test_smalltalk_non_whitelist_action_json_gets_fallback(admin_token, session_with_steps, mock_projection, mock_route_intent_smalltalk):
