...
export default function NotesPanel({
  ...
  const canGenerate = !!sessionId && !isLocalSessionId(sessionId) && typeof onGenerate === "function";
      ...
      <div className="panelBody">
        <div className="card" style={{ marginBottom: 12 }}>
          <div style={{ fontWeight: 900, marginBottom: 6 }}>
            {title ? String(title) : "Сессия"}
          </div>

          <div className="small muted" style={{ marginBottom: 10 }}>
            id: <span style={{ fontWeight: 900 }}>{sessionId || "—"}</span>
          </div>
        ...
        <button
          ...
          title={
            !canGenerate
              ? "Для генерации нужен проект + API-сессия (TopBar → выбери проект → Новая (API))"
              : ""
          }
        >
        ...
        {!canGenerate ? (
          <div className="small muted" style={{ marginTop: 12 }}>
            Подсказка: для генерации процесса нужна API-сессия (TopBar → выбери проект → “Новая (API)”).
          </div>
        ) : null}
      </div>
    </div>
