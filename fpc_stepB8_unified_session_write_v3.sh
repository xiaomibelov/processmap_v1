...
TS="$(date +%F_%H%M%S)"
TAG="cp/fpc_stepB8_unified_write_v3_start_${TS}"
git tag -a "$TAG" -m "checkpoint: step B8 v3 start (${TS})" >/dev/null 2>&1 || true

PY="$HOME/fpc_patch_stepB8_unified_write_v3.py"
cat > "$PY" <<'PY'
import re, json
...
# typing imports best-effort
if "from typing import" not in s or ("Any" not in s and "Optional" not in s):
...
DEC_RE = re.compile(
    r"(?m)^\s*@(?P<obj>\w+)\.(?P<meth>get|post|patch|put)\(\s*(?P<q>['\"])(?P<path>[^'\"]+)(?P=q)[^)]*\)\s*$"
)

decorators = []
for m in DEC_RE.finditer(s):
    path = m.group("path")
    if "/api/" in path and "sessions" in path:
        decorators.append((m.group("obj"), m.group("meth"), path))

def pick_path(meth: str, want: str):
    for obj, mm, path in decorators:
        if mm != meth:
            continue
...
obj_get_one, path_get_one = pick_path("get", "single")
obj_get_list, path_get_list = pick_path("get", "list")
obj_post,     path_post     = pick_path("post", "list")

obj_patch, path_patch = (None, None)
if any(mm=="patch" and re.search(r"sessions/\{", p) for _,mm,p in decorators):
    obj_patch, path_patch = pick_path("patch", "single")

obj_put, path_put = (None, None)
if any(mm=="put" and re.search(r"sessions/\{", p) for _,mm,p in decorators):
    obj_put, path_put = pick_path("put", "single")

if not obj_patch or not path_patch:
    raise SystemExit("No PATCH /api/sessions/{...} endpoint detected in main.py. Stop.")
...
'''
    m = re.search(r"(?m)^\s*\w+\s*=\s*FastAPI\(", s)
    if not m:
...
# PUT: replace if exists, else insert after patch block
...
# Replace get/post to always dump notes as array
...
DOC.write_text(f'''# Session API contract (frontend bridge)
...
- `GET {path_get_list}` -> list[Session]
- `POST {path_post}` -> Session
- `GET {path_get_one}` -> Session
- `PATCH {PATH_SINGLE}` -> Session (partial OR full session-shape)
- `PUT {PATH_SINGLE}` -> Session (replace whole session, frontend fallback if PATCH not supported)
- `GET /api/sessions/{{id}}/bpmn` -> `application/xml` (bpmn-js viewer)

## Input normalization (accepted aliases)
...
- notes:
  ...
  - backend stores internally as string, but API returns list
...
RESP="$(curl -sS -X POST "http://127.0.0.1:${HOST_PORT}/api/sessions" \
  ...
  -d '{"title":"b8v3_smoke","roles":[{"role_id":"cook_1","label":"Повар 1"},{"role_id":"technolog","label":"Технолог"}]}')"
...
TAG2="cp/fpc_stepB8_unified_write_v3_done_${TS}"
git tag -a "$TAG2" -m "checkpoint: step B8 v3 done (${TS})" >/dev/null 2>&1 || true
