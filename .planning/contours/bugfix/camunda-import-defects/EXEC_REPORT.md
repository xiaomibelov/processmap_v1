...
- Implementation commits:
  - `46dc90e2 fix(import): preserve camunda task data`
  - `c9881e17 fix(import): keep subprocess wrappers and duplicate properties`
...
2. Camunda properties were preserved only as a compatibility `Dict[str, str]`.
   ...
   - Code review found exact duplicate property rows were still dropped by `extract_camunda_extensions_from_bpmn_xml`. Fix: row IDs are now ordinal-aware at `camunda_meta_utils.py:70` and rows are no longer deduplicated before output (`camunda_meta_utils.py:317`).
...
4. File import/export initially preserved nested task names but flattened away the `bpmn:subProcess` wrapper.
   - Code review found `generate_bpmn` emitted all parsed nodes directly under `bpmn:process`.
   - Fix: importer records subprocess wrapper metadata at `bpmn_import.py:327` and `bpmn_import.py:641`; exporter reconstructs wrappers at `bpmn_export.py:165`, emits child nodes/flows into the matching wrapper at `bpmn_export.py:296` and `bpmn_export.py:375`, and adds wrapper DI at `bpmn_export.py:448`.
...
- `backend/tests/fixtures/camunda_nested_task_properties.bpmn`
  ...
  - duplicate and exact-duplicate `camunda:property name="ingredient"` rows;
...
- `backend/tests/test_camunda_import_defects.py`
  - file/import-template path: `parse_bpmn` preserves top-level and nested task names, ordered properties including exact duplicate rows, raw inputOutput, and `generate_bpmn -> parse_bpmn` round-trip preserves names/properties and the `subProcess` wrapper.
...
Code review:
- `gsd-code-reviewer` reported 2 blockers after the first implementation:
  - generated round-trip flattened away `bpmn:subProcess`;
  - exact duplicate `camunda:property` rows were still dropped.
- Both were fixed in `c9881e17`, and the same targeted backend gate above passed afterwards.
...
3. Round-trip:
   - `import -> export -> import` preserves top-level and nested task names, exact duplicate ordered `camunda:property` rows, raw `camunda:inputOutput`, and the tested `bpmn:subProcess` wrapper.
4. Deployment:
   - Not done.
