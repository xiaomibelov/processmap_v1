...
## Фикс после живой приёмки (2026-08-14, коммит 2c11fe3d)

Живая проверка пользователем на :5199 выявила блокер: на ширинах <1044px
колонка «Название» схлопывалась до min-content (~30px) — table-fixed отдавал
всю ширину фиксированным колонкам. Программная сверка DOM этого не ловила;
диагностировано замерами getBoundingClientRect на реальных данных (978 проектов).

Фикс: явная сетка в коде — `minWidth` таблицы 1044px (784 fixed + 260 «Название»,
1116px с сигнальными колонками) + контейнер `overflow-auto` (горизонтальный
скролл ниже 1044px вместо схлопывания). Полное скрытие колонок по брейкпоинтам
ТЗ — Фаза 2 (container queries).

Верификация: build OK; unit 111/1 (pre-existing); скриншоты с РЕАЛЬНЫХ данных
`live-{1440,1100,880,640}.png` + замеры: nameW = 350 @1440 (flex), 260 @1100/880/640.
Визуальный чек-лист пройден глазами: названия читаемы, налезаний нет.

Процессное правило (принято): к каждой фазе — скриншоты 1440/1100/880/640 +
визуальный чек-лист («названия читаемы полностью», «нет налезаний») глазами
