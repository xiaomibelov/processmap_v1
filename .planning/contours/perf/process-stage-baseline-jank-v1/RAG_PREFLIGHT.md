...
- **generated_at**: 2026-05-16T19:54:38.226Z
...
- **snippet**:
```
[contour: *processmap*-agent-rag-bm25-manifest-search-v1] ```bash grep -E '"path".*\.env' .*planning*/contours/feature/*processmap*-agent-rag-bm25-manifest-search-v1/RAG_MANIFEST_SAMPLE.json || echo "PASS: no .env" grep -E '"path".*\.pem' .*planning*/contours/feature/*processmap*-agent-rag-bm25-manifest-search-v1/RAG_MANIFEST_SAMPLE.json || echo "PASS: no .pem" grep -E '"path".*node_modules' .*planning*/contours/feature/*processmap*-agent-rag-bm25-m…
```
...
```
- ```bash
node tools/rag/pm-rag-agent-preflight.mjs --role planner --contour "perf/process-stage-baseline-jank-v1" --area "ProcessMap planning context" --format md
```
- ```bash
