...
- Десятичная запятая (`"0,5"`, `"0,66"`) **нормализуется** в точку по решению владельца от 21/08/26, но только для строгого паттерна `^\d+,\d+$` (ровно одна запятая, цифры вокруг). Многоэтажные запятые (`"1,2,3"`) и префиксы (`"> 10"`) остаются текстом (случай B).
- Нормализация применяется к `ingredient_value` и к `ee_time`.
