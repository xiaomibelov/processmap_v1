...
- **area/query**: rag_search fullcorpus window / list_rag_chunks ORDER BY created_at ASC LIMIT fullcorpus window glossary hit@3
- **generated_at**: 2026-09-07T18:51:11.171Z
...
- ```bash
node tools/rag/pm-rag-search.mjs "rag_search fullcorpus window" --top-k 5
```
- ```bash
node tools/rag/pm-rag-search-facts.mjs "rag_search fullcorpus window" --top-k 8 --json
```
- ```bash
node tools/rag/pm-rag-agent-preflight.mjs --role planner --contour "fix/rag-fullcorpus-window-v1" --area "rag_search fullcorpus window" --format md
```
- ```bash
