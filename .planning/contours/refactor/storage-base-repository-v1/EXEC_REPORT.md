...
⚠️ **Честная фиксация по приёмке ≥2500**: цель сформулирована в метрике «клон-строк» аудита (сумма длин клон-инстансов по файлам; compat 1213 + org_auth 630 + ai 514 + notes 361 + audit 87 ≈ 2805). Отслеживаемая нами метрика jscpd `duplicatedLines` — это уникальные дублированные строки домена (было 2568 всего), она консервативнее. По ней суммарно возможный максимум контура — ≤2568, фактически достигнуто −369. До ≥2500 по метрике аудита предлагается либо (а) пересчитать дельту тем же скриптом, чтоим аудит, и приложить цифру, либо (б) продлить контур на DDL/агрегаты compat (вне текущих границ). Требование принято решение владельца.
...
- **code:** ветка `refactor/storage-base-repository-v1`, HEAD `ea61273a`, 12 коммитов, diffstat: 18 файлов, +13071/−1064 (с учётом артефактов контура).
