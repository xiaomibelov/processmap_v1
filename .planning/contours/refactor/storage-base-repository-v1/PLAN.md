...
1. notes → коммит (jscpd-дельта шага в сообщении)
2. audit_telemetry → коммит (jscpd-дельта)
3. ai → коммит (jscpd-дельта)
4. org_auth → коммит (jscpd-дельта)
5. compat → коммит (jscpd-дельта + **таблица семантических расхождений** `_row_to_dict`/`_now_ts` из п. 3A.1)
6. `PLAN.md` + `STATE.json` + handoff (planning-коммит)

## 3A. Условия approve (2026-09-14)

1. **Таблица семантических расхождений — до шага 5 (compat), в PLAN и в коммите compat-шага.** Аудит: 6 разных `_row_to_dict`, 5 вариантов `_now_ts`. Обязательная таблица: call-site → исходный вариант → каноническая версия → доказательство эквивалентности (или задокументированное отличие). Без неё канонизация = тихая смена поведения.
2. **jscpd-дельта в каждом коммите шага** (не только в финале): замер `npx jscpd backend/app/domains/storage` до/после каждого шага, дельта фиксируется в сообщении коммита и STATE.json. Суммарная приёмка: ≥ 2500 клон-строк. Если шаг дельту не даёт — **стоп и пересмотр**, а не продолжение.
3. **Шаг = коммит = независимо зелёный.** Если шаг 3 (ai) или 4 (org_auth) падает и не чинится быстро — контур фиксируется на последнем зелёном шаге и уходит на PR в таком объёме. Пошаговый откат — режим работы, не абстракция.
