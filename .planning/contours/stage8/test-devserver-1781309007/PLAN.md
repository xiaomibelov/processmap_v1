# Contour: stage8/test-devserver-1781309007
...
3. If unreachable, record the failure verbatim and set `worker_status` to `complete` with a clear environment note (do **not** modify product code to start the server).
...
- API test launcher: `.agents/tests/stage8_api_test.sh`
...
| Risk | Mitigation |
|------|-----------|
| Dev server not running | Report environment-dependent unreachable status; do not patch product code to start it |
...
## 8. Handoff Marker
Agent 2 waits for: `READY_FOR_EXECUTION` directory present in `.planning/contours/stage8/test-devserver-1781309007/`
