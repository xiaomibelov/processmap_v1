...
## Input
- **role**: planner
- **contour**: audit/authz-impl
- **area/query**: backend authorization granular permissions implementation / permissions_json org_membership ORG_WRITE_ROLES ORG_READ_ROLES is_role_allowed can_edit_workspace authz backend routers
- **generated_at**: 2026-06-21T19:00:54.846Z
...
- ```bash
node tools/rag/pm-rag-search.mjs "backend authorization granular permissions implementation" --top-k 5
```
- ```bash
node tools/rag/pm-rag-search-facts.mjs "backend authorization granular permissions implementation" --top-k 8 --json
```
- ```bash
node tools/rag/pm-rag-agent-preflight.mjs --role planner --contour "audit/authz-impl" --area "backend authorization granular permissions implementation" --format md
```
- ```bash
