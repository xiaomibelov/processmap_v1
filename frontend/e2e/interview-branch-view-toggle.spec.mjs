...
async function ensureGatewayBranchRow(page) {
  ...
  const txt = String((await toggle.textContent()) || "").trim().toLowerCase();
  if (txt.includes("развернуть")) {
    await toggle.click();
  }
  return betweenRow;
}

test("Interview branch view toggle switches tree/cards and persists after reload", async ({ page, request }) => {
  ...
  const betweenRow = await ensureGatewayBranchRow(page);
  await page.getByTestId("interview-branch-view-cards-btn").click();
  await expect(page.getByTestId("interview-branch-view-cards-btn")).toHaveClass(/isActive/);
  await expect(betweenRow.getByTestId("interview-branches-cards")).toBeVisible();

  await page.waitForTimeout(200);
  await page.reload();
  await openSession(page, sessionMeta.projectId, SESSION_ID);

  const betweenRowAfterReload = await ensureGatewayBranchRow(page);
  await expect(page.getByTestId("interview-branch-view-cards-btn")).toHaveClass(/isActive/);
  await expect(betweenRowAfterReload.getByTestId("interview-branches-cards")).toBeVisible();

  await page.getByTestId("interview-branch-view-tree-btn").click();
  await expect(page.getByTestId("interview-branch-view-tree-btn")).toHaveClass(/isActive/);
  await expect(betweenRowAfterReload.getByTestId("interview-branches-tree")).toBeVisible();
});
