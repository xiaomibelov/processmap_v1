  ...
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    <bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="Process_1">
      ...
      <bpmndi:BPMNShape id="Task_1_di" bpmnElement="Task_1">
        <dc:Bounds x="280" y="145" width="180" height="86" />
      </bpmndi:BPMNShape>
...
async function createFixture(request, runId, headers) {
  const projectRes = await request.post(`${API_BASE}/api/projects`, {
    headers,
    data: { title: `E2E notes template ${runId}`, passport: {} },
  });
  ...
  const sessionRes = await request.post(
    `${API_BASE}/api/projects/${encodeURIComponent(projectId)}/sessions?mode=quick_skeleton`,
    {
      headers,
      data: {
        title: `E2E notes template session ${runId}`,
...
test("notes templates: insert template for selected element and persist notes_by_element", async ({ page, request }) => {
  ...
  await page.getByTestId("notes-template-insert").click();
  ...
  expect(String(items[0]?.text || "")).toContain("Роль/ответственный");
  await expect(page.locator("#element-notes-section")).toContainText("Шаблон: User Task");
});
