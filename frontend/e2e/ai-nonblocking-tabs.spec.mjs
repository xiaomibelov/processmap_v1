...
function seedBpmnXml(name = "E2E AI nonblocking") {
...
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <bpmn:process id="Process_1" isExecutable="false" name="${name}">
    ...
    <bpmn:task id="Activity_1" name="AI nonblocking task" />
  ...
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    <bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="Process_1">
      ...
      <bpmndi:BPMNShape id="Activity_1_di" bpmnElement="Activity_1">
        <dc:Bounds x="300" y="118" width="150" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="EndEvent_1_di" bpmnElement="EndEvent_1">
        <dc:Bounds x="540" y="140" width="36" height="36" />
      </bpmndi:BPMNShape>
    </bpmndi:BPMNPlane>
  </bpmndi:BPMNDiagram>
</bpmn:definitions>`;
...
async function createFixture(request, runId) {
  const projectRes = await request.post(`${API_BASE}/api/projects`, {
    data: { title: `E2E AI nonblocking project ${runId}`, passport: {} },
  });
  ...
  const sessionRes = await request.post(
    ...
    {
      data: {
        title: `E2E AI nonblocking session ${runId}`,
  ...
  const putRes = await request.put(`${API_BASE}/api/sessions/${encodeURIComponent(sessionId)}/bpmn`, {
    data: { xml: seedBpmnXml(`E2E AI nonblocking ${runId}`) },
  });
...
test("AI fail не блокирует switch tab и не ломает Diagram", async ({ page, request }) => {
  ...
  let recomputeRequests = 0;
  await page.route(/\/api\/sessions\/[^/]+\/recompute$/, async (route) => {
    recomputeRequests += 1;
    await route.fulfill({
      status: 500,
      contentType: "application/json",
      body: JSON.stringify({ error: "forced ai recompute fail" }),
    });
  });

  await openFixture(page, fixture);
  await expect.poll(async () => probeReady(await readProbe(page))).toBeTruthy();

  const generateBtn = page.getByRole("button", { name: "Сгенерировать процесс" }).first();
  ...
  await switchTab(page, "Diagram");
  await expect.poll(async () => probeReady(await readProbe(page))).toBeTruthy();
  await expect(page.locator(".segBtn.on")).toContainText("Diagram");
  expect(recomputeRequests).toBeGreaterThan(0);
});
