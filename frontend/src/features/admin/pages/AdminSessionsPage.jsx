...
export default function AdminSessionsPage({
  ...
  const filterItems = [
    ...
    { label: "Owners", value: filters?.ownerIds },
    { label: "Attention", value: filters?.attentionOnly ? "yes" : "" },
    { label: "Range", value: filters?.updatedRange },
  ];
  ...
  return (
    <AdminPageContainer
      ...
      secondary={(
        <div className="grid gap-4 xl:grid-cols-2">
          <SectionCard title="Requires Attention" subtitle="Highest pressure sessions from current filters" eyebrow="Attention">
            <div className="space-y-3">
              ...
              )) : <div className="text-sm text-slate-500">No risky sessions in current slice.</div>}
            </div>
          </SectionCard>
          <SectionCard title="Redis Distribution" subtitle="Redis OFF states are degraded/incident, not normal" eyebrow="Persistence">
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3">
                <div className="text-xs text-amber-700">Fallback</div>
              ...
              <div className="rounded-2xl border border-rose-200 bg-rose-50 p-3">
                <div className="text-xs text-rose-700">Incident</div>
    ...
    >
      <AdminFiltersBar title="Sessions Filters" subtitle="Filter by query, status, owner, and diagnostic emphasis" activeFilters={filterItems}>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          <AdminSearchInput value={toText(filters?.q)} onChange={(value) => setPatch({ q: value })} placeholder="Search session / project / org" testId="admin-sessions-q" />
          <AdminSelectFilter
          ...
          />
          <AdminSearchInput value={toText(filters?.ownerIds)} onChange={(value) => setPatch({ ownerIds: value })} placeholder="Owner ids, comma-separated" />
          <AdminDateRangeFilter value={toText(filters?.updatedRange)} onChange={(value) => setPatch({ updatedRange: value })} />
          <AdminToggleFilter checked={Boolean(filters?.attentionOnly)} onChange={(value) => setPatch({ attentionOnly: value })} label="Attention only" />
        </div>
      </AdminFiltersBar>
