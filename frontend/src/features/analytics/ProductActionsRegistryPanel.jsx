...
function Metric({ label, value, accent = false }) {
  return (
    <span className="productActionsRegistrySummaryPill" data-accent={accent ? "incomplete" : null}>
      {value}
      <b>{label}</b>
    </span>
  );
  ...
  return (
    <div className={page ? "productActionsRegistryPanel productActionsRegistryPanel--page" : "productActionsRegistryPanel"} data-testid="product-actions-registry-panel">
                ...
                >
                  ...
                  <div className="productActionsRegistrySessionActions">
                    <a
                      ...
                      style={{ color: "#7C3AED", fontSize: 12, fontWeight: 500, textDecoration: "none", opacity: toText(item.project_id) ? 1 : 0.4, pointerEvents: toText(item.project_id) ? "auto" : "none" }}
                    ...
                    <a
                      ...
                      style={{ color: "#7C3AED", fontSize: 12, fontWeight: 500, textDecoration: "none", opacity: toText(item.session_id) ? 1 : 0.4, pointerEvents: toText(item.session_id) ? "auto" : "none" }}
      ...
      <section className="productActionsRegistrySummary" aria-label="Сводка реестра">
        <Metric label="Сессий" value={visibleSessionTotal} />
        <Metric label="Строк" value={summary.rows} />
        <Metric label="Полных" value={summary.complete} />
        <Metric label="Неполных" value={summary.incomplete} accent={summary.incomplete > 0} />
        <Metric label="После фильтров" value={filteredSummary.rows} />
      </section>
      ...
      <section className="productActionsRegistryFilters" data-testid="product-actions-registry-filters">
        <div className="productActionsRegistryFiltersToolbar">
          ...
          {FILTERS.some(([key]) => toText(filters[key])) || (filters.completeness || "all") !== "all" ? (
            <button
              type="button"
      ...
      {sessionViewModel?.analysis?.product_actions?.ai_suggestions?.length ? (
        <div className="productActionsRegistryAiRow" data-testid="product-actions-registry-ai-suggestions">
          <span style={{ fontSize: 16 }}>✦</span>
          <span>AI предлагает {sessionViewModel.analysis.product_actions.ai_suggestions.length} улучшений</span>
          <button type="button" className="secondaryBtn smallBtn">Показать рекомендации</button>
        </div>
      ) : null}
      ...
      {sessionViewModel?.analysis?.product_actions?.source_state?.sources?.length ? (
        <section className="productActionsRegistrySources" data-testid="product-actions-registry-sources">
          <div className="productActionsRegistrySourcesTitle">Источники данных</div>
          {sessionViewModel.analysis.product_actions.source_state.sources.map((source) => (
            <div className="productActionsRegistrySourceItem" key={source.name || source.id}>
              <span className="productActionsRegistrySourceDot" />
