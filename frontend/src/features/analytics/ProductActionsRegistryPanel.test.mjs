...
const pageSource = fs.readFileSync(path.join(__dirname, "ProductActionsRegistry.jsx"), "utf8");
const productPanelSource = fs.readFileSync(path.join(__dirname, "../../components/process/interview/ProductActionsPanel.jsx"), "utf8");
...
test("ProductActionsRegistryPanel preserves fallback when backend fields are absent", () => {
  assert.match(source, /normalizeBackendRows\(vmResult\?\.analysis\?\.product_actions\?\.rows\)/);
