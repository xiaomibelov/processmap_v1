...
test("properties overlay decor binds to preview element id and renders rows for non-empty values", () => {
  ...
  withDocumentStub(() => {
    ...
    assert.match(secondRow.style.getPropertyValue("--fpc-property-muted-text"), /^hsl\(\d+ 32% 28%\)$/);
    assert.match(secondRow.style.getPropertyValue("--fpc-property-separator"), /^hsl\(\d+ 28% 40%\)$/);
  });
});
