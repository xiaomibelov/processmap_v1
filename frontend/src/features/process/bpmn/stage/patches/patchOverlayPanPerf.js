// Patch diagram-js Overlays to skip per-overlay visibility/scale updates
// while the overlay root is hidden during panning. This eliminates the
// O(n) DOM write storm that drops FPS to <10 on diagrams with 50+
// property overlays.
...
export function patchOverlaysPrototype() {
  ...
  try {
    ...
    Overlays.prototype._updateOverlaysVisibilty = function (viewbox) {
      total += 1;
      if (this._overlayRoot && this._overlayRoot.style.visibility === "hidden") {
        skipped += 1;
...
export function patchOverlaysInstance(inst) {
  ...
  try {
    ...
    overlays._updateOverlaysVisibilty = function (viewbox) {
      if (this._overlayRoot && this._overlayRoot.style.visibility === "hidden") {
        return;
      }
      return original(viewbox);
    };
