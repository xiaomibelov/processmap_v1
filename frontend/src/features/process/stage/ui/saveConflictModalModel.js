...
export function buildSaveConflictModalView({
  ...
  return {
    ...
    contextLines,
    actions: {
      refreshLabel: "Обновить и продолжить",
      refreshHint: "Загрузить последнюю серверную версию. Локальные несохранённые изменения будут заменены.",
      overwriteLabel: "Перезаписать мои изменения",
      overwriteHint: "Сохранить вашу локальную версию поверх серверной (база = текущая серверная версия). Изменения из другого окна будут перезаписаны; действие фиксируется в истории.",
      stayLabel: "Отмена",
      stayHint: "Закрыть окно и продолжить редактирование без обновления. Сохранения останутся на паузе до решения.",
      ...
      compareLabel: "Сравнить и выбрать",
      compareHint: "Открыть side-by-side сравнение версий и выбрать, какую сохранить.",
    },
  };
}
