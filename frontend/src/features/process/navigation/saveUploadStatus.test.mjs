...
test("badge hides non-blocking uploading chatter in user-facing toolbar", () => {
...
test("badge hides unchanged skip success telemetry", () => {
  const badge = buildSaveUploadStatusBadge({
...
test("badge shows failed status with http code when available", () => {
  ...
  assert.match(badge.label, /Ошибка сохранения BPMN/i);
  assert.equal(badge.code, "HTTP 409");
