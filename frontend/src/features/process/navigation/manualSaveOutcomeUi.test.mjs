...
test("manual save success + companion failure is surfaced as secondary warning", () => {
  const ui = resolveManualSaveOutcomeUi({
    primarySaveOk: true,
    primarySavePending: false,
    companionError: "meta sync failed",
    saveInfo: "Сохранено внутри версии.",
  });

  assert.equal(ui.primaryState, "primary_saved_companion_warning");
  assert.equal(ui.genErr, "");
  assert.match(ui.infoMsg, /Сохранено внутри версии\./);
  assert.match(ui.infoMsg, /метаданные версии пока не синхронизированы/i);
...
test("manual save failure surfaces primary error", () => {
  const ui = resolveManualSaveOutcomeUi({
    primarySaveOk: false,
...
test("manual save meta-only stale retry does not show alarming version notice", () => {
  const ui = resolveManualSaveOutcomeUi({
    primarySaveOk: true,
    staleRetryApplied: true,
    staleRetryChangedKeys: ["bpmn_meta"],
  });
