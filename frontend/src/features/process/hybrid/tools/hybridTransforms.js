...
export function clampHybridRectSize(rectRaw, options = {}) {
...
export function applyHybridDragDelta(rectRaw, deltaRaw) {
  const rect = clampHybridRectSize(rectRaw);
  const delta = asObject(deltaRaw);
  return {
    x: round1(Number(rect.x || 0) + Number(delta.dx || 0)),
    y: round1(Number(rect.y || 0) + Number(delta.dy || 0)),
    w: round1(rect.w),
    h: round1(rect.h),
  };
}
...
export function applyHybridResizeHandleDelta(rectRaw, handleRaw, deltaRaw, options = {}) {
  const rect = clampHybridRectSize(rectRaw, options);
