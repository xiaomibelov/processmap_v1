...
test("app update hook silently hides banner on failed or missing meta and never auto reloads", () => {
  ...
  assert.ok(hookSource.indexOf("reloadPage(window);") > hookSource.indexOf("const refresh"));
});

test("app update banner has the expected copy and normal reload/dismiss actions", () => {
