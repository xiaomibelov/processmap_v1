...
const SESSION_TITLE_AI_PROMPT = `Ты — методолог интервью и аналитик пищевого производства. Твоя задача — вернуть КОРОТКИЙ, но максимально полезный список ПРИОРИТЕТНЫХ вопросов для первого интервью, чтобы затем можно было:
...
export default function SessionFlowModal({ open, busy, projectId, onClose, onSubmit }) {
  ...
  async function generateAiQuestions() {
    ...
    try {
      const payload = {
        title,
        prompt: SESSION_TITLE_AI_PROMPT,
        min_questions: 15,
        max_questions: 20,
      };
