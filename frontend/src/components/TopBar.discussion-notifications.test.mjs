...
test("TopBar exposes bounded discussion notification entry from existing note aggregate truth", () => {
  ...
  assert.match(topBarSource, /Требует внимания/);
  assert.match(topBarSource, /data-notes-panel-trigger="true"/);
  assert.match(topBarSource, /onOpenDiscussionNotifications\?\.\(\)/);
  assert.match(topBarSource, /attentionDiscussionCount = Math\.max\(0, Number\(notesAggregate\?\.attention_discussions_count \|\| 0\)/);
  assert.match(topBarSource, /accountNotificationCount = mentionCount \+ attentionDiscussionCount/);
  assert.match(topBarSource, /Упоминания и внимание/);
  assert.match(topBarSource, /Обсуждения требуют внимания: \$\{attentionDiscussionCount\}/);
