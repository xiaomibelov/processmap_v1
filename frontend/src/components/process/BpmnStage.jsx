...
// fix/canvas-apply-persist-validation: FpcAlignDiagramHandler (общий с
// align), applyGeometryOnInstance и apply-экстрактор плана живут в
// canvasGeometryApplyInstance.js — headless-тестируемый bpmn-js-вайринг
// без React (интеграционные тесты apply→persist→undo на real-фикстуре).
import {
  FpcAlignDiagramHandler,
  applyGeometryOnInstance,
} from "../../features/process/bpmn/layout/canvasGeometryApplyInstance.js";
...
async function alignDiagramOnInstance(inst, options = {}) {
  ...
  try {
    ...
    // Сериализация не удалась → сбой persist-пути: откатываем команду,
    // align не остаётся применённым (контракт undo-on-persist-failure #1043;
    // fix/canvas-apply-persist-validation, FIX B).
    let saved;
    try {
      saved = await inst.saveXML({ format: true });
    } catch (saveError) {
      try { commandStack.undo(); } catch { /* undo guard */ }
      return { ok: false, error: String(saveError?.message || saveError || "align_serialize_failed") };
    }
    ...
    // Сохранение не удалось → откатываем команду, align не применяется.
    ...
    if (persistXml) {
      ...
      // ok:true без bpmnVersionSnapshot — штатный no-op guard сервера (XML
      // уже закоммичен, версия-снапшот создана конкурентным писателем) —
      // это УСПЕХ persist'а, undo ложный
