...
TS="$(date +%F_%H%M%S)"
BR="feat/frontend-r4-bpmn-viewer-v1"
TAG_START="cp/foodproc_frontend_r4_start_${TS}"
TAG_DONE="cp/foodproc_frontend_r4_done_${TS}"
ZIP_DIR="artifacts"
ZIP_PATH="${ZIP_DIR}/foodproc_frontend_r4_${TS}.zip"

echo
echo "== checkpoint tag (start) =="
git tag -a "$TAG_START" -m "checkpoint: frontend R4 start (${TS})" >/dev/null 2>&1 || true
echo "$TAG_START"
...
echo
echo "== deps: install bpmn-js =="
( cd frontend && npm -s install bpmn-js )

echo
echo "== write BpmnStage =="
...
echo
echo "== update ProcessStage to use BpmnStage + controls =="
...
echo
echo "== update AppShell: pass sessionId into ProcessStage =="
...
git add -A frontend
git status -sb || true
git commit -m "feat(frontend): add bpmn-js viewer (R4) load /api/sessions/{id}/bpmn + fit/zoom" >/dev/null 2>&1 || true

echo
echo "== checkpoint tag (done) =="
git tag -a "$TAG_DONE" -m "checkpoint: frontend R4 done (${TS})" >/dev/null 2>&1 || true
echo "$TAG_DONE"
